...
from loss_functions import loss_kcenter, loss_modularity, loss_kcenter_myloss
...
loss_kcenter_zeroth = loss_kcenter_myloss.apply
...
if run_decision:
    for t in range(args.train_iters):
        ...
        #link prediction setting: get loss with respect to training edges only
            ...
            print(dist.shape)
            loss = loss_fn(dist, train_object, args)
        # print(r.shape)
        # assert 1==2
        if args.objective != 'kcenter' or args.objective != 'kcenter_zeroth':
            loss = -loss
        ...
        if t % 100 == 0:
            ...
            loss_test = loss_fn(dist, test_object, args)
